...
export function FuelRecordForm({
  ...
  // 編集時: バックエンドから返される UTC 文字列を JST に変換
  // 新規作成時: 空文字列
  const getInitialDatetime = () => {
    if (!initialData?.refuel_datetime) return ''
    // UTC 文字列をパースしてブラウザのローカルタイムゾーン（JST）に変換
    const date = new Date(initialData.refuel_datetime)
    // datetime-local 用の形式 "YYYY-MM-DDTHH:mm" に変換
    return format(date, "yyyy-MM-dd'T'HH:mm")
  }
  const [refuelDatetime, setRefuelDatetime] = useState(getInitialDatetime())
  const [totalMileage, setTotalMileage] = useState(
    initialData?.total_mileage?.toString() ?? '',
  )
  ...
  const [totalCost, setTotalCost] = useState(
    initialData?.total_cost?.toString() ?? '',
  )
  const [isFullTank, setIsFullTank] = useState(
    initialData?.is_full_tank ?? false,
  )
  const [gasStationName, setGasStationName] = useState(
    initialData?.gas_station_name ?? '',
  )
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // datetime-local の値（ローカルタイムゾーン = JST）を UTC に変換
    const localDate = new Date(refuelDatetime)
